    ...
    def check_balance(self, address, api_url_template, api_key='', network_name=''):
        ...
        try:
            ...
            # Tratamento para erros de limite de taxa
                ...
                logging.error(
                    f"Erro ao obter saldo para {address} na rede {network_name}: {data.get('message', 'Nenhuma mensagem')}")
                return 0
    ...
    try:
        while True:
            ...
            if generator.check_btc:
                ...
                btc_balance = generator.check_balance(btc_address, "https://blockchain.info/balance?active={address}",
                                                      network_name="BTC")
                if btc_balance > 0:
            ...
            if generator.check_eth:
                ...
                eth_balance = generator.check_balance(
                    eth_address,
                    "https://api.etherscan.io/api?module=account&action=balance&address={address}&tag=latest&apikey=" + os.getenv(
                        "ETHERSCAN_API_KEY"),
                    network_name="ETH"
                )
                if eth_balance > 0:
            ...
            if generator.check_bsc:
                bsc_balance = generator.check_balance(
                    eth_address,
                    "https://api.bscscan.com/api?module=account&action=balance&address={address}&apikey=" + os.getenv(
                        "BSCSCAN_API_KEY"),
                    network_name="BSC"
                )
                if bsc_balance > 0:
            ...
            if generator.check_avax:
                avax_balance = generator.check_balance(
                    eth_address,
                    "https://api.snowtrace.io/api?module=account&action=balance&address={address}&tag=latest&apikey=",
                    # Aqui, inclua a chave da API se necessário
                    network_name="AVAX"
                )
                if avax_balance > 0:
            ...
            if generator.check_matic:
                matic_balance = generator.check_balance(
                    eth_address,
                    "https://api.polygonscan.com/api?module=account&action=balance&address={address}&apikey=" + os.getenv(
                        "MATICSCAN_API_KEY"),
                    network_name="MATIC"
                )
                if matic_balance > 0:
